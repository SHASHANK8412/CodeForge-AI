...
export default function CodeWorkspace({ generationId = 'aiforge-demo', setView }) {
  ...
  return (
    <div className="h-full w-full bg-[#090d16] text-slate-100 font-sans flex flex-col min-h-0 min-w-0 overflow-hidden select-none">
      ...
      {diffModalData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-6 select-text">
          <div className="bg-[#0b1329] border border-slate-700/80 rounded-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl">
            <div className="px-5 py-4 border-b border-slate-800 bg-slate-950 flex items-center justify-between text-slate-200">
              <div>
                <h3 className="text-sm font-bold font-mono">Proposed Change — {diffModalData.file}</h3>
                <p className="text-[10px] text-slate-500 font-mono mt-0.5">Please review the diff below before applying the fix.</p>
              </div>
              <button onClick={() => setDiffModalData(null)} className="p-1 text-slate-400 hover:text-white rounded hover:bg-slate-800 transition">
                <FaTimes className="w-3.5 h-3.5" />
              </button>
            </div>
            <div className="flex-1 p-5 overflow-y-auto font-mono text-xs bg-[#070b13] custom-scrollbar text-slate-300">
              {diffModalData.diff ? (
                diffModalData.diff.split('\n').map((line, idx) => {
                  let className = "text-slate-400";
                  if (line.startsWith('+')) className = "text-emerald-400 bg-emerald-500/10 px-1 rounded-sm";
                  else if (line.startsWith('-')) className = "text-rose-400 bg-rose-500/10 px-1 rounded-sm";
                  return (
                    <div key={idx} className={`whitespace-pre-wrap leading-relaxed py-0.5 font-mono ${className}`}>
                      {line}
                    </div>
                  );
                })
              ) : (
                <div className="text-slate-500 italic">No diff content generated.</div>
              )}
            </div>
            <div className="px-5 py-4 border-t border-slate-800 bg-slate-950 flex items-center justify-end gap-3 shrink-0">
              <button onClick={() => setDiffModalData(null)} className="px-4 py-2 bg-slate-850 hover:bg-slate-800 text-slate-300 rounded-lg text-xs font-semibold cursor-pointer border border-slate-800">
                Reject
              </button>
              <button onClick={handleApplyFix} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 cursor-pointer">
                <FaCheck /> Apply Fix
              </button>
            </div>
          </div>
