...
UNIVERSAL_SYSTEM_PROMPT = """You are AIForge's Explanation Agent.
Your job is to explain concepts accurately, clearly, and naturally.
Understand the user's requested depth.

For beginner or general questions:
- Use simple, clear language.
- Define key terms and use real-world analogies.
- Structure complex topics logically.

For technical questions:
- Explain how and why the concept works.
- Include relevant technical details appropriate to the request.

CRITICAL BOUNDARIES:
- Do NOT generate source code unless explicitly requested by the user.
- Do NOT use competitive programming algorithm templates.
- NEVER output competitive programming section headers (such as problem approach, algorithmic approach, solver functions, or complexity analysis) for non-code explanations.
- Do NOT transform general concepts into algorithms."""
...
class ExplanationAgent:
    ...
    def _generate_real_domain_response(self, prompt: str, domain: str) -> str:
        ...
        if "bfs" in p_lower or "dfs" in p_lower or "graph" in p_lower:
            return (
                "## Understanding BFS vs DFS Graph Traversal\n\n"
                "**Breadth-First Search (BFS)** and **Depth-First Search (DFS)** are fundamental graph and tree traversal algorithms.\n\n"
                "### Breadth-First Search (BFS)\n"
                "- **Traversal Order**: Level-by-level (explores all neighbors at current depth before moving deeper).\n"
                "- **Data Structure**: Uses a **Queue** (FIFO).\n"
                "- **Use Cases**: Finding shortest path in unweighted graphs.\n\n"
                "### Depth-First Search (DFS)\n"
                "- **Traversal Order**: Explores as far along each branch as possible before backtracking.\n"
                "- **Data Structure**: Uses a **Stack** (LIFO) or Recursion.\n"
                "- **Use Cases**: Topological sorting, cycle detection, pathfinding in mazes."
            )

        elif "chennai super kings" in p_lower or "csk" in p_lower:
            return (
        ...
        elif "airplane" in p_lower or "airplanes" in p_lower or "fly" in p_lower:
            return (
                "## How Airplanes Fly\n\n"
                "Airplanes stay in the air thanks to four fundamental aerodynamic forces: **Lift**, **Thrust**, **Drag**, and **Weight**.\n\n"
                "### The 4 Aerodynamic Forces\n"
                "- **Lift**: Generated by the wings (airfoils) using **Bernoulli's Principle** and Newton's Third Law. As the curved wing moves through air, air moves faster over the top, creating lower pressure above the wing and pushing the airplane upward.\n"
                "- **Thrust**: Provided by jet engines or propellers to propel the airplane forward through the air.\n"
                "- **Drag**: Air resistance opposing the forward motion.\n"
                "- **Weight (Gravity)**: The downward force pulling the airplane toward Earth.\n\n"
                "When Lift overcomes Weight and Thrust overcomes Drag, an airplane climbs and cruises smoothly through the sky!"
            )

        elif "10-year-old" in p_lower or "10 year old" in p_lower or ("photosynthesis" in p_lower and "child" in p_lower):
            return (
                "## What is Photosynthesis? (Explained Simply)\n\n"
                "Imagine plants have a tiny secret kitchen inside their green leaves!\n\n"
                "### How Plants Make Food\n"
                "1. **Sunlight**: Leaf solar panels catch warm light from the sun.\n"
                "2. **Water**: Plant roots drink water up from the soil.\n"
                "3. **Air (Carbon Dioxide)**: Leaves breathe in carbon dioxide gas from the air.\n\n"
                "Using solar power, the leaf mixes water and air together to cook delicious **glucose** (plant sugar) for food, and releases fresh **oxygen** into the air for us to breathe!"
            )

        elif "photosynthesis" in p_lower:
            return (
                "## What is Photosynthesis?\n\n"
                "**Photosynthesis** is the biological process used by plants, algae, and cyanobacteria to convert solar energy into chemical energy stored in glucose.\n\n"
                "### Biological Mechanism\n"
                "- **Equation**: $6\\text{CO}_2 + 6\\text{H}_2\\text{O} + \\text{Light} \\rightarrow \\text{C}_6\\text{H}_{12}\\text{O}_6 + 6\\text{O}_2$\n"
                "- **Chloroplasts**: Organelles containing chlorophyll pigments that absorb light energy.\n"
                "- **Outputs**: Glucose (for plant growth and energy) and Oxygen (released as a byproduct)."
            )
        ...
        # Call LLM service if available for other queries
        try:
        ...
        # Fallback response without fake template headers!
        formatted_title = prompt.strip().rstrip("?").title()
        return (
            f"## {formatted_title}\n\n"
            f"**{formatted_title}** is a subject in the domain of **{domain}**.\n\n"
            f"### Overview & Context\n"
            f"- **Overview**: Core concepts, history, and real-world significance of {formatted_title}.\n"
            f"- **Structure**: Primary attributes and operational framework associated with {formatted_title}."
        )
